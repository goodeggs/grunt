...
file.copy = function(srcpath, destpath, options) {
  ...
  if (contents === false) {
    ...
    if (options.preservePermissions) {
      file.setPermissions(destpath, srcpath);
    }
  }
};

// Set user + group and/or permissions on a file, given specific uid, gid,
// mode options OR a source path from which to clone these values.
file.setPermissions = function(filepath, options) {
  // If options is a filepath, get that file's uid, gid and mode.
  if (typeof options === 'string') {
    if (!file.exists(options)) {
      throw grunt.util.error('Source path "' + options + '" doesn\'t exist.');
    }
    options = fs.statSync(options);
  } else if (!options) {
    options = {};
  }

  var nowrite = grunt.option('no-write');
  var setOwnerGroup = options.uid && options.gid;
  var setPermissions = options.mode;

  // Output a meaningful message.
  var msgParts = [];
  if (setOwnerGroup) { msgParts.push('owner', 'group'); }
  if (setPermissions) { msgParts.push('permissions'); }
  var message = (nowrite ? 'Not actually setting ' : 'Setting ') +
    msgParts.join(' + ') + ' on ' + filepath + '...';
  ...
  if (!nowrite) {
    if (setOwnerGroup) {
      try {
        ...
        grunt.verbose.error();
        throw grunt.util.error('Error while setting owner and group of "' + filepath + '" file.', e);
      }
    }
    if (setPermissions) {
      try {
        ...
        grunt.verbose.error();
        throw grunt.util.error('Error setting permissions of "' + filepath + '" file.', e);
      }
    }
  }
  grunt.verbose.ok();
